...
export default function ChatBox() {
	...
	useEffect(() => {
		const fetchHistory = async () => {
			const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/Messages/history`);
      ...
      `}>
							...
							{msg.role === "user" && (
								<button
									...
									className="absolute -top-1 -left-6 opacity-0 group-hover:opacity-100 transition-opacity text-gray-400 hover:text-blue-600"
									title="Chỉnh sửa"
								>
